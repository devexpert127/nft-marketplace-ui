...
import { CopySvg, ProfileAvatarSvg, ProfileBanner } from '../atlas-assets';
...
const Collection = ({ match }) => {
  ...
  const getAllNfts = async publicKey => {
    let response = await getAndReturnApiData(`nft/getNFTs`, publicKey);
  };

  const getCollectionNfts = async _ => {
    let response = await getAndReturnApiData(
      `nft/getNftByCollection`,
      match.params.id,
    );
